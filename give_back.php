...
<body>
  ...
  <?php include './slidebar.php'; ?>
    ...
    $sql_EQ = 'SELECT borrow.*, equipment.*, admin.* FROM  `borrow`  
                  INNER JOIN equipment ON borrow.borrow_eqID = equipment.EqID 
                  INNER JOIN admin ON borrow.borrow_admin = admin.admin_no
    ...
    ?>  
      ...
      <thead>
        <tr>
          <th <?php if(isset($_SESSION['borrow'])){ echo 'colspan="6"'; }else{ echo 'colspan="5"';} ?>><?=$result_name['stu_name']?></th>
        </tr>
    ...
    <?php while($result_EQ = $stm2->fetch(PDO::FETCH_BOTH)){ ?>
        <tr class="center aligned">
          ...
          <td><?=$result_EQ['admin_name']?></td>
          <?php if(isset($_SESSION['borrow'])){ ?>
          <td>
            <a class="ui teal button" href="./sql/return.php?id=<?=$result_EQ['borrow_no']?>">คืนแล้ว</a>
          </td>
          <?php } ?>
        </tr>
      </tbody>
